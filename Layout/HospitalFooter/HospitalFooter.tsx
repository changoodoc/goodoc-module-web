...
const HospitalFooter = (
  DividerVertical: any,
  VersionTooltip: any = Empty,
  FooterMenualFiles: any = Empty
) => {
  return ({ isLogin = true, isFix = false }: { isLogin?: boolean; isFix?: boolean }) => {
    return (
          ...
          <section className="Footer__section1">
            <div className="Footer__col1">
              <div className="Footer__col1--section2">
                ...
                <div className="Footer__info">
                  {[HOSPITAL_FOOTER_TXT.addr, HOSPITAL_FOOTER_TXT.allRight].map((item, idx) => {
                    return (
                      <span key={idx}>
                        {item}
                        {idx === 1 && (<VersionTooltip /> || '')}
                        <br />
                      </span>
                    );
                  })}
            ...
            <div className={`Footer__col2${isLogin ? '' : ' bottom'}`}>
              <div className="Footer__col2--section1">
                <FooterMenualFiles isLogin={isLogin} />
              </div>
            </div>
          </section>
        </div>
