...
describe('DeepLink 객체를', () => {
  test('병원정보보기', () => {
    ...
    const a = DeepLink.create('hospitals', { contentId: 1234 }).setFunnel(
      'web_hospitaldetail_button'
    );
    expect(a.value).toBe('goodoc%3A%2F%2Fhospitals%2F1234%3FFunnel%3Dweb_hospitaldetail_button');
    expect(decodeURIComponent(a.value)).toBe(
      'goodoc://hospitals/1234?Funnel=web_hospitaldetail_button'
    );
  });
  test('유의사항 더보기 버튼', () => {
